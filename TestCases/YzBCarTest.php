...
require_once dirname(__FILE__).'/../CommonClass/car.php';


class YzBCarTest extends BaseCase{
	//测试夜租48元套餐，包括我要还车页面，还车成功页面，还车详情页面
	...
	function  test_get_car_info(){
		...
		for ($i=0; $i < $carnums; $i++) {
			...
			if($this->result['status']!=0){
				echo "YzBCarTest:::"."choose_car/index.html:::".$this->rlt['msg'] ;
				return False ;
			}
			...
			if($this->rlt['status']==0){
				...
				if($i<$carnums){
					$n = $i + 1 ;
					echo "YzBCarTest:::"."第".$n."辆返回提示::".$this->rlt['msg'] .'。 ' ;
					continue ;
				}else{
					return False ;
				}
	...
	// 	//print_r($this->parms) ;
	// 	$this->rlt=$this->post_reslut($this->url,$this->parms);
	// 	$this->print_log('YzBCarTest.php','test_chooseyzcar',$this->rlt,0) ;
	...
	// 	$this->rlt=$this->post_reslut($this->url,$this->parms);
	// 	$this->print_log('YzBCarTest.php','test_yzchoosesubmit',$this->rlt,0) ;
	// 	$car_orderid = $this->rlt['data']['order_id']; 
	...
	public function test_opencardoor($carinfo){
		...
		$this->parms['city_id'] = city_id;
		$this->rlt=$this->post_reslut($this->url,$this->parms);
		$this->print_log('YzBCarTest.php','test_opencardoor',$this->rlt,0) ;
		$this->assertEquals($this->rlt['msg'],"开车门成功");
		update_dist_remain(self::$conn,0,$carinfo[0],2) ;	//修改里程>110km
	...
	public function test_usecarfee($carinfo){
		...
		$this->parms['city_id'] = city_id;
		$this->rlt=$this->post_reslut($this->url,$this->parms);
		$this->print_log('YzBCarTest.php','test_usecarfee',$this->rlt,0) ;
		...
		$this->assertEquals($this->rlt['data']['activity_money'],MAX_PRICE) ;
		$this->assertEquals($this->rlt['data']['activity_rule'],activity_rule1) ;
		$this->assertEquals($this->rlt['data']['activity_type'],activity_type) ;
		$this->assertEquals($this->rlt['data']['charge_total'],MAX_PRICE) ;
	...
	public function test_backcar($carinfo){
		...
		//print_r($this->parms);
		$this->rlt=$this->post_reslut($this->url,$this->parms);
		$this->print_log('YzBCarTest.php','test_backcar',$this->rlt,0) ;
	 ...
	 * @depends test_get_car_info
	 */
	public function test_yzBtourdetail($carinfo){
		...
		$this->parms['city_id'] = city_id;
		$this->rlt=$this->post_reslut($this->url,$this->parms);
		$this->print_log('YzBCarTest.php','test_yzBtourdetail',$this->rlt,0) ;
		$this->assertEquals($this->rlt['data']['charge_total'],MAX_PRICE) ;
		$this->assertEquals($this->rlt['data']['youhui_yuee'],MAX_PRICE) ;
		$this->assertEquals($this->rlt['data']['charge_actual'],charge_actual) ;
		$this->assertEquals($this->rlt['data']['charge_activity']['money'],MAX_PRICE) ;	//优惠券id
